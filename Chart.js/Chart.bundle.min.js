...
function doDrill(buttonid, hidfieldid,c, i, chart) {
	e = i[0];
	if (e && e._index != null) {
		var x_value = chart.data.labels[e._index];
		var y_value = chart.data.datasets[0].data[e._index];
		doDrill1(buttonid, hidfieldid,e._index, y_value, x_value);
	}
}
...
function getColors(numColors, startColor, scheme, variation, distance,nonRandomColors) {
	scm = new ColorScheme;
	compliment = false;
	if (!startColor) startColor = Math.round(Math.random() * 360);
	if (!scheme) scheme = 'analogic';
	if (scheme == 'analogic_compliment') { scheme = 'analogic'; compliment = true; }
	if (!variation) variation = 'pastel';
	if (!distance) distance = 0.5;
	if ((startColor + '').length <= 3) {
		scm.from_hue(startColor);
	} else {
		scm.from_hex(startColor);
	}
	
	scm.scheme(scheme)
		.distance(distance)
		.add_complement(compliment)
		.variation(variation);
	var c = scm.colors().map(function (hex) { return '#' + hex; });
	if (!nonRandomColors)
		c = shuffle(c);
	while (c.length < numColors)
		c = c.concat(c.slice(0));
	return c;
}
...
Chart.dropShadow = false;
Chart.shadowColor = "#000000";
Chart.shadowBlur = 15;
Chart.shadowOffsetX = 5;
Chart.shadowOffsetY = 5;

Chart.gradient = false;
Chart.gradientStart = 1.5;
Chart.gradientEnd = 0.5;
Chart.gradientStartShade = 0;
Chart.gradientEndShade = 0.8;
...
Chart.elements.Rectangle.prototype.draw = function () {
    ...
    if (ctx._dropShadow == null) ctx._dropShadow = Chart.dropShadow;
    if (ctx._dropShadow) {
        if (ctx._shadowColor == null) ctx._shadowColor = Chart.shadowColor;
        if (ctx._shadowBlur == null) ctx._shadowBlur = Chart.shadowBlur;
        if (ctx._shadowOffsetX == null) ctx._shadowOffsetX = Chart.shadowOffsetX;
        if (ctx._shadowOffsetY == null) ctx._shadowOffsetY = Chart.shadowOffsetY;
        ctx.dropShadow = true;
        ctx.shadowColor = ctx._shadowColor;
        ctx.shadowBlur = ctx._shadowBlur;
        ctx.shadowOffsetX = ctx._shadowOffsetX;
        ctx.shadowOffsetY = ctx._shadowOffsetY;
	}
	// Set Radius Here
	// If radius is large enough to cause drawing errors a max radius is imposed
    var cornerRadius = ctx.cornerRadius;
    if (cornerRadius==null) cornerRadius = Chart.cornerRadius;
	...
	//ctx.save();
	if (ctx.gradient) {
		var grd = ctx.createLinearGradient(x, y + height * Chart.gradientStart, x, y + height * Chart.gradientEnd);
		//if (height < 0) grd = ctx.createLinearGradient(0, height * 0.2, 0, height * 1.5 );
		var currentcolor = ctx.fillStyle;
		var rgbcolor = hexToRgbA(currentcolor);
		grd.addColorStop(1, shadeRGBColor(rgbcolor, Chart.gradientStartShade));
		grd.addColorStop(0, shadeRGBColor(rgbcolor, Chart.gradientEndShade));
		ctx.fillStyle = grd;
	}
	ctx.fill();
